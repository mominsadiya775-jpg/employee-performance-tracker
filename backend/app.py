    ...
    try:
        ...
        for row in rows:
            try:
                ...
                total_receivable = 0
            entries.append({"id": row[0], "entry_date": str(row[1]), "month": row[2], "customer_name": row[3], "data_managed_by": row[4], "coordination_done_by": row[5], "total_receivable": total_receivable, "status": row[7]})
