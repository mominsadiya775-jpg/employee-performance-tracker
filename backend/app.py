...
app.config['MYSQL_PASSWORD'] = 'sadiya2006'
app.config['MYSQL_DB'] = 'employee_tracker'

# PRODUCTION READY (UNCOMMENT LATER)
...
def delete_entry(id):
    ...
    try:
        ...
        query = "DELETE FROM ledger_entries WHERE id=%s"
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            values = (
                ...
                str(row.get('Type of support'))
                if pd.notna(row.get('Type of support'))
                else None,

                str(row.get('Data Managed By'))
                if pd.notna(row.get('Data Managed By'))
                else None,

                str(row.get('Coordination done by'))
                if pd.notna(row.get('Coordination done by'))
                else None,
                ...
                str(row.get('Remarks if any'))
                if pd.notna(row.get('Remarks if any'))
                else None
            )
